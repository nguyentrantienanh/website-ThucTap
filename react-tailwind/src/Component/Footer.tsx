...
function Footer() {
    ...
    const address = t('footer information.address');
    const phone = t('footer information.phone');
  return (
    <footer className=" border-t-2 border-[#ffffff] w-auto">
        <div className="grid grid-cols-3 max-[770px]:grid-cols-2 max-[580px]:grid-cols-1 gap-7   text-[#fff] bg-[#6e4925] justify-between   px-50  py-10   max-[1450px]:px-20   max-[1030px]:px-0   ">
            <section className="flex flex-col gap-4 max-[1450px]:gap-3 text-[21px] max-[1450px]:text-[14px] px-5 ">
                ...
                <p className="text-[15px]">{t('footer information.company')}</p>
                <div className=" border-t-3 border-[#fff] w-[15%]"></div>
                    ...
                    <a href="">
                        <IconFC name="clock" />
                        <span className="ml-2">{t('footer information.working hours')}</span>
                    </a>
            </section>
            <section className="flex flex-col   gap-4 max-[1450px]:gap-3 text-[21px] max-[1450px]:text-[14px] px-5">
                <h1 className="font-medium text-[28px] max-[1450px]:text-[20px] ">{t('footer quick link.title')}</h1>
                <div className=" border-t-3 border-[#fff] w-[30%]"></div>
                <a href="">{t('footer quick link.about')}</a>
                <a href="">{t('footer quick link.gallery')}</a>
            </section>
            <section className="flex flex-col gap-2 px-5">
                <h1 className="font-medium text-[28px] max-[1450px]:text-[20px]">{t('footer Connect')}</h1>
                <div className=" border-t-3 border-[#fff] w-[30%]"></div>
